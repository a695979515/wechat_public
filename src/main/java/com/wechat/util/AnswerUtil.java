...
public class AnswerUtil {
     ...
     * @param toUserName 接收方帐号（收到的OpenID）
     * @param fromUserName 开发者微信号
     * @param newsEntities 图文回复实体类
     * @return
     */
    public String NewsAnswer(String toUserName, String fromUserName, List<NewsEntity> newsEntities) {
        StringBuffer sb = new StringBuffer();
        if (newsEntities != null && newsEntities.size() > 0 && newsEntities.size() <= 10) {
            Date data = new Date();
            sb.append("<xml>");
            sb.append("<ToUserName><![CDATA[" + toUserName + "]]></ToUserName>");
            sb.append("<FromUserName><![CDATA[" + fromUserName + "]]></FromUserName>");
